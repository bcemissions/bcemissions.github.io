...
Raphael.fn.g.piechart = function (cx, cy, r, values, opts) {
    ...
    if (len == 1) {
        ...
        // This sorting mechanism works against the way I want the pie to be displayed. --Carson Lam
        ...
        for (i = 0; i < len; i++) {
        	// Angles here tweaked so that stacked pies align. Start clockwise at 0rad. --Carson Lam
            var mangle = angle * values[i] / total / 2;
            if (!i) {
                angle = 0;
                mangle = angle * values[i] / total / 2;
            }
            if (opts.init) {
